...
from .export_blobs import _make_azure_client, _make_gcs_client, _make_s3_client, export_blobs_for_run
...
def run_backup(
    ...
    *,
    ...
    azure_client = _make_azure_client() if include_blobs and SETTINGS.blob_backend_type == "azure" else None
    ...
    for i, rh in enumerate(hashes, 1):
        ...
        if include_blobs and (s3_client is not None or gcs_client is not None or azure_client is not None):
            bc, bb = export_blobs_for_run(
                rh,
                backup_dir,
                s3_client=s3_client,
