...
Uses the same FDB isolation pattern as storage tests.
"""
...
def _isolated_directories(monkeypatch: pytest.MonkeyPatch) -> Generator[Directories, None, None]:
    ...
    prefix = ("_test_workers", test_id)

    runs_dir = _fdb_directory.create_or_open(db, (*prefix, "data", "runs"))
    indexes = _fdb_directory.create_or_open(db, (*prefix, "data", "indexes"))
    system = _fdb_directory.create_or_open(db, (*prefix, "system"))
    dirs = fdb_client.Directories(runs=runs_dir, indexes=indexes, system=system)
